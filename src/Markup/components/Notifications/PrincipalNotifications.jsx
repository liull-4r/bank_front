...
const PrincipalNotifications = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://bank-system-back.onrender.com/detection/usertoprincipalnotification/?principal_id=${user?.user_id}`
        );
  ...
  const markAsRead = async (notificationId) => {
    try {
      await axios.patch(
        `https://bank-system-back.onrender.com/detection/usertoprincipalnotification/${notificationId}/`,
        {
          read: true,
        }
      );
          ...
          {notifications.map((notification) => (
            <div key={notification.id} style={styles.notification}>
              {notification.message.startsWith("New Message") ? (
                <Link
                  to={`/principaldetail/${extractAppointmentId(
                ...
                <Link
                  to={`/principalresponsedetail/${extractAppointmentId(
                    notification.message
                  )}`}
                >
                  <p>{removeIdFromMessage(notification.message)}</p>
